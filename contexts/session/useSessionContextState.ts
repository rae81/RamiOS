...
import { type ApiError } from "utils/browserfsCompat";
    ...
    if (!initializedSession.current && rootFs) {
      const initSession = async (): Promise<void> => {
        ...
        try {
          ...
          const migrateLegacyWallpaper =
            session.wallpaperImage === "VANTA" ||
            session.wallpaperImage === "VANTA ALT";
          const sessionWallpaperImage =
            !session.wallpaperImage || migrateLegacyWallpaper
              ? DEFAULT_WALLPAPER
              : session.wallpaperImage;
          ...
          setWallpaper(sessionWallpaperImage, session.wallpaperFit);
          if (
            session.sortOrders &&
